...
def MSELoss_Vector(pred, target):
    return torch.sqrt(torch.sum((pred - target) ** 2, dim=-1))

def OrthoLoss(pred_X_axis, pred_Y_axis, pred_Z_axis):
    XY = torch.abs(torch.sum(pred_X_axis*pred_Y_axis, dim=-1))
    YZ = torch.abs(torch.sum(pred_Y_axis*pred_Z_axis, dim=-1))
    ZX = torch.abs(torch.sum(pred_Z_axis*pred_X_axis, dim=-1))
    return XY + YZ + ZX
...
class YOLOLoss(nn.Module):
    ...
    def forward(self, input, targets=None):
        #--------------------------------------------------------------#
        #   input shape:    bs, 3*(5+num_classes+num_poses*3), 13, 13
        #                   bs, 3*(5+num_classes+num_poses*3), 26, 26
        #                   bs, 3*(5+num_classes+num_poses*3), 52, 52
        #--------------------------------------------------------------#
        ...
        #----------------------------------------------------------------------------------------#
        # bs, 3*(5+num_classes+num_poses), 13, 13 -> bs, 3, 13, 13, (5+num_classes+num_poses*3)
        # bs, 3*(5+num_classes+num_poses), 26, 26 -> bs, 3, 13, 13, (5+num_classes+num_poses*3)
        # bs, 3*(5+num_classes+num_poses), 52, 52 -> bs, 3, 13, 13, (5+num_classes+num_poses*3)
        #----------------------------------------------------------------------------------------#
        prediction = input.view(bs, int(self.num_anchors / 3), 
            self.bbox_attrs + self.num_poses*3, in_h, in_w).permute(0, 1, 3, 4, 2).contiguous()
        ...
        # poses
        pred_yaw = torch.tanh(prediction[..., 6:9])
        pred_pitch = torch.tanh(prediction[..., 9:12])
        pred_roll = torch.tanh(prediction[..., 12:])
        ...
        loss_yaw = torch.sum(MSELoss_Vector(pred_yaw, yaw) * mask)
        loss_pitch = torch.sum(MSELoss_Vector(pred_pitch, pitch) * mask)
        loss_roll = torch.sum(MSELoss_Vector(pred_roll, roll) * mask)
        loss_Ortho = torch.sum(OrthoLoss(pred_yaw, pred_pitch, pred_roll) * mask)
        ...
        losses = {
            ...
            "w"         : loss_w.item() / bs,
            "h"         : loss_h.item() / bs,
            "confidence": loss_conf.item() / bs,
            "class"     : loss_cls.item() / bs,
            "yaw"       : loss_yaw.item() / bs,
            "pitch"     : loss_pitch.item() / bs,
            "roll"      : loss_roll.item() / bs,
            "ortho"     : loss_Ortho.item() / bs,
            "bbox attr" : self.lambda_xy * (loss_x + loss_y) + self.lambda_wh * (loss_w + loss_h) \
                                            + self.lambda_conf * loss_conf + self.lambda_cls * loss_cls,
            "poses"     : self.lambda_yaw * loss_yaw + self.lambda_pitch * loss_pitch + self.lambda_roll * loss_roll +loss_Ortho
        }
    ...
    def get_target(self, target, anchors, in_w, in_h, ignore_threshold):
        ...
        yaw     = torch.zeros(bs, int(self.num_anchors / 3), in_h, in_w, 3, requires_grad=False)
        pitch   = torch.zeros(bs, int(self.num_anchors / 3), in_h, in_w, 3, requires_grad=False)
        roll    = torch.zeros(bs, int(self.num_anchors / 3), in_h, in_w, 3, requires_grad=False)
        ...
        for b in range(bs):
            ...
            for i, best_n in enumerate(best_ns):
                ...
                if (gj < in_h) and (gi < in_w):
                    ...
                    # pose
                    yaw[b, best_n, gj, gi, :] = target[b][i, 5:8]
                    pitch[b, best_n, gj, gi, :] = target[b][i, 8:11]
                    roll[b, best_n, gj, gi, :] = target[b][i, 11:]
                else:
