...
import cv2
import os
from utils.utils import convertListAngleToVector
...
class YoloDataset(Dataset):
    def __init__(self, train_lines, image_size, is_train):
        ...
        ], bbox_params=A.BboxParams(format='coco', min_visibility=0.6))
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=0.1, hue=0.1, sat=1.5, val=1.5, random=True):
        ...
        if not random:
            ...
            if len(box) > 0:
                ...
                if len(box_data > 0):
                    box_data[:, 4:] = box_data[:, 4:] * np.pi / 180.0
                    poses_vector = convertListAngleToVector(box_data[:, 4], box_data[:, 5], box_data[:, 6])
                    classes = np.zeros((len(box_data), 1))
                    box_data = np.concatenate((box_data[:, :4], classes, poses_vector), axis=1)
