...
class DecodeBox(nn.Module):
    ...
    def forward(self, input):
        ...
        yaw = torch.tanh(prediction[..., 5])
        pitch = torch.tanh(prediction[..., 6])
        roll = torch.tanh(prediction[..., 7])
        ...
        output = torch.cat((pred_boxes.view(batch_size, -1, 4) * _scale,
                            conf.view(batch_size, -1, 1), yaw.view(batch_size, -1, 1), pitch.view(batch_size, -1, 1), roll.view(batch_size, -1, 1)), -1)
        return output.data
...
def non_max_suppression(prediction, num_classes, conf_thres=0.5, nms_thres=0.4):
    ...
    for image_i, image_pred in enumerate(prediction):
        ...
        class_conf = image_pred[:, 5:5 + num_classes]
