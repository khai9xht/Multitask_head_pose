...
class YoloBody(nn.Module):
    def __init__(self, config):
        ...
        self.num_poses = 3 # yaw, pitch, roll
        self.num_anchors = 3 # number of anchors for each scale
        out_filter_cv = self.num_anchors * self.num_poses
        self.cv2 = conv2d(out_filters[-4]+out_filters[-3], out_filter_cv, 1)
        self.cv1 = conv2d(out_filters[-3]+out_filters[-2], out_filter_cv, 1)
    ...
    def forward(self, x):
        ...
        pose1_branch = self.cv1(x1_in)
        pose2_branch = self.cv2(x2_in)
...
if __name__ == "__main__":
    import sys
    import os
    sys.path.append('/media/2tb/Hoang/multitask/code/yolo_linear')
    sys.path.append('/media/2tb/Hoang/multitask/code/yolo_linear/nets')
    os.chdir("/media/2tb/Hoang/multitask/code/yolo_linear/nets")
