...
def object_detection_collate(batch):
    ...
    pose_type = type(batch[0][3])
    for image, boxes, labels, poses in batch:
        ...
        if pose_type is np.ndarray:
            gt_poses.append(torch.from_numpy(poses))
        elif pose_type is torch.Tensor:
            gt_poses.append(poses)
        else:
            raise TypeError(f"Boxes should be tensor or np.ndarray, but got {pose_type}.")

    return torch.stack(images), gt_boxes, gt_labels, gt_poses
